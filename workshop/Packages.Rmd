...
`search` will show what packages are currently loaded in memory and also some other items. 

```{r }
search()
```

These other items include data frames loaded with `attach()` as well as any environments which exist. Both those items may be confusing for the novice. One may also use `.packages()`. Output is returned invisibly, so easiest to capture that in a variable.
