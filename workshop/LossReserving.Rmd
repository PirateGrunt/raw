...
```{r }
data(PPA)
View(PPA_LossDevelopment)
library(reshape2)
PPA_LossDev <- dcast(PPA_LossDevelopment, AccidentYear ~ Lag, sum)
row.names(PPA_LossDev) <- PPA_LossDev$AccidentYear
PPA_LossDev$AccidentYear <- NULL
PPA_LossDev <- as.matrix(PPA_LossDev)
PPA_LossDev[PPA_LossDev == 0] <- NA

#PPA_LossDev <- PPA_LossDev[-(1:2), ]
myFit <- ClarkLDF(PPA_LossDev, maxage=Inf)
```

## 

TODO: add a bootstrap
...
dfDisplay = as.data.frame(op)[, 1:3]
