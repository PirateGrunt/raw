...
\begin{frame}[fragile]{Predicting results}
Most models predict results using the 'predict' function. By default, a prediction is made against the sample data. It's possible to predict against new observations.
<<eval=TRUE>>=
yHat = predict(myFit)
@
We'll use this more in the next few slides.
\end{frame}
...
lines(X1, yHat)
@
<<size='tiny', fig.height=4.5, echo=FALSE>>=
plot(X1, Y, pch=19)
lines(X1, yHat)
@
\end{frame}
